...
class Main(QWidget):
    def __init__(self):
        ...
        self.title = 'English Myanmar Dictionary v2'
        self.top = 100
        self.left = 200
        self.width = 600
        self.height = 400
    ...
    def __config(self):
        ...
        if (not config or config['is_max']):
            self.showMaximized()
        else:
            config.pop('is_max', -1)
            self.width, self.height, self.left, self.top = config.values()
    ...
    def __set_config(self):
        ...
        if self.isMaximized():
            config = {'width': width, 'height': height,
                      'left': left, 'top': top, 'is_max': True}
        else:
            config = {'width': width, 'height': height,
                      'left': left, 'top': top, 'is_max': False}
        try:
